...
import { CustomSamlClient } from "./samlClient";
...
export type XmlTamperer = (xml: string) => Promise<string>;
...
export class SpidStrategy extends SamlStrategy {
  ...
  constructor(
    ...
    this.extendedRedisCacheProvider = getExtendedRedisCacheProvider(
      this.redisClient,
      Math.floor(options.requestIdExpirationPeriodMs / 1000)
    );
    ...
    return this.getSamlOptions(req, (err, samlOptions) => {
      ...
      return this.tamperMetadata
        ? // Tamper the generated XML for service provider metadata
          this.tamperMetadata(originalXml).then(tamperedXML => callback(null, tamperedXML))
